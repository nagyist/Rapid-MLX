...
def test_default_policy_is_stock_with_explicit_experiment_modes(monkeypatch):
    monkeypatch.delenv("MLX_QWEN4_FUSED_EXPERT_KERNEL", raising=False)
    assert qwen4_exp._qwen4_fused_expert_mode_from_env() == "stock"
    for value in ("0", "off", "false", "stock", "unexpected"):
        monkeypatch.setenv("MLX_QWEN4_FUSED_EXPERT_KERNEL", value)
        assert qwen4_exp._qwen4_fused_expert_mode_from_env() == "stock"
    for value in ("auto", "scalar", "tile4"):
        monkeypatch.setenv("MLX_QWEN4_FUSED_EXPERT_KERNEL", value)
        assert qwen4_exp._qwen4_fused_expert_mode_from_env() == value


def test_kernel_admits_only_qualified_decode_widths():
    for tokens in (1, 3):
        admission = qwen4_fused_moe.admit_qwen4_fused_down(*_production_inputs(tokens))
...
def test_auto_selects_real_weight_winners_by_token_width():
    ...
    for tokens, expected in ((1, "tile4"), (3, "scalar")):
        ...
        with (
            patch.object(qwen4_exp, "QuantizedSwitchLinear", FakeQuantizedDown),
            patch.object(qwen4_exp, "admit_qwen4_fused_down", return_value=admission),
            patch.object(
