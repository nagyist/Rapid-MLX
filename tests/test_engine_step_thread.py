...
class TestMLLMSchedulerStepThread:
    ...
    def test_prefix_cache_clear_waits_behind_inflight_step(self):
        """An admin clear must run after an already-started MLLM step.

        Otherwise that step can store a prefix after the clear returns and the
        endpoint's successful response is immediately false.
        """
        import concurrent.futures
