...
def test_live_installer_handles_empty_pressure_and_optional_base_hooks(monkeypatch):
    ...
    # This contract exercises optional base hooks, not the B1 driver.  Keep
    # the fixture explicitly ineligible so the new one-lane route cannot make
    # its deliberately skeletal runtime observable.
    batch_gen._unprocessed_sequences.append(
        _scheduler_sequence(1, processors=[object()])
    )
    config = SchedulerConfig(
        spec_decode="mtp",
        mtp_continuous_batching=True,
    ...
    no_getter._unprocessed_sequences.append(
        _scheduler_sequence(2, processors=[object()])
    )
    assert _install_continuous_mtp_router(
        no_getter,
        _Model(),
