...
enum ModelCatalog {
    ...
    /// Video aliases now use the same task-driven catalog path as every other
    /// model kind. The Settings tab can expose them without a second parser or
    /// an alias allowlist; older sidecars simply return an empty video list.
    static func videoEntries(
        ...
        return mergeAtomicAndCached(
            atomic: atomic, cached: await cachedTask, excluded: []
        ).filter { $0.kind == .video }
    }
